...
const PACKAGE = `angular-typed-storage`;
const NPM_DIR = `dist`;
...
exec(`node build-scripts/map-sources -f ${MODULES_DIR}/${PACKAGE}.js`);
...
exec(`node build-scripts/map-sources -f ${MODULES_DIR}/${PACKAGE}.es5.js`);
rm(`-f`, `${MODULES_DIR}/${PACKAGE}.es5.ts`);

echo(`Run Rollup conversion on package`);
exec(`rollup -c rollup.config.js --sourcemap`, {silent: true});
exec(`node build-scripts/map-sources -f ${BUNDLES_DIR}/${PACKAGE}.umd.js`);
...
exec(`node ../../build-scripts/map-sources -f ${PACKAGE}.umd.min.js`);
cd(`..`);
cd(`..`);
